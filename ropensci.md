...
While we are supposedly entering the era of big data, the problem for much of science is that large-scale, global databases must be assembled from a collection of independent and  heterogenous fragments -- the outputs of many and isolated scientific studies conducted around the globe.
...
Together with 95 co-authors, we recently published the Biomass And Allometry Database (BAAD), combining data from 176 different scientific studies into a single unified database. We built BAAD for several reasons: i) we needed it for our own work ii) we perceived a strong need within the vegetation modelling community for such a database and iii) because it allowed us to road-test some new methods for building and maintaing a database[^database].

Until now, every other data compilation we are aware of has been assembled in the dark. By this we mean, end-users are provided with a finished product, but remain unaware of the diverse modifctaions that have been made to components in assembling the unified database. Thus users have limited insight into the quality of methods used, nor are they able to build on the compilation themselves.

The apporach we took with BAAD is quite different: our database is built from raw inputs using scripts; plus the entire workflow is available for users to inspect, run themselves and ultimately build upon. We believe this is a better way for managing lots of #otherpeoplesdata and so below share some of the key insghts from our experience.
...
The only potential cost of continually rebuilding the database is that the process of rebuilding can take time. Actually, the time taken to make all the transformations and combine all 176 studies is pretty minimal ~9s allup. Never-the-less, the job of continually rebuilding the database was made a lot quicker through use of [remake](https://github.com/richfitz/remake)[^remake], one of our new R packages. Essentially remake caches built objects (e.g. the transformed data from each study) and only rebuilds each of them if either the data or code generating that particular dataset has changed. So if I change a single value in one of the datasets, remake notices that and rebuilds only that file. So after the first longer run, rebuilding the entire database takes in the range of 1-2s.
...
Each study in the database was therfore required to have a standard set of files to enter the data-processing pipeline:
...
- `dataNew.csv`: allows for additon of any new data not present in `data.csv`, or modfication of existing values based on a find and replace logic.
...
There are several important benfits to this appraoch of seperating code from data:

- it is highly scalable.
- it separates data from code, so that potentially someoone could replace the R code using the exact same data.
- it drastically reduces the amount of R code needed.

## Establish a formal process for processing and reviewing each data set

We established a system for tracking the progress of each dataset enetering BAAD

1. Initial screening (basic metatdata extracted from paper).
2. Primary authors contacted (asking if they wish to contribute).
3. Initial response from authors (inidcating interest or not)
4. Email sent requesting raw data from authors.
5. Raw data received from authors.
6. Data processed and entered into BAAD (we filled out as much of infomration as we could ourselves).
7. A review of data, including any queries, sent to authors for error checking.
9. Data approved (finish).
10. Data excluded because of issues that arose (no response, not ineterested, could not locate data, data not suitable etc)  (finish).

At each stage we automated as much as possible. We used a script to genertae emails in R based on information in our database, and made it as easy as possible for the contributors to fulfill their tasks and get back to us.
...
Figure: Example figure showing porblematic data.

# 3. Use version control (git) to track changes and code sharing wesbite (github) for effective collaboration

The BAAD project began in July 2012, in Feb 2013 Rich FitzJohn got involved and introduced us to version control. You can see the structure of our database at that time [here](https://github.com/dfalster/baad/tree/912163bb371e280340dee2bb4cf872a1d7ede81b). We can't recall that much about what happended prior 13 Feb 2013, but since that day, every single change to the BAAD has been recorded. We know who changed what lines of code or data and when. Many people have been extolling the virtues of git for managing computer code (e.g. [Chacon 2009](http://git-scm.com/book)), but others have noted that git is equally good for managing data ([Ram et al 2013](http://doi.org/10.1186/1751-0473-8-7)).
...
- the raw datafiles and metadata are made available for others to reuse in new and different contexts,
- the data is immediately available on the web, without need to register or login into a site, or submit a project approval.

Anyone can use the compiled data in whatever way they see fit. Our goal was to create a database that many scinetists would immediately want to use, and that would therfore get cited a lot.

Another concern was that the database would be sustianble. By making the entire process open and scripted, we are effectively allowing oursleves to step away from the project at some point in the future, if that's what we want to do.
